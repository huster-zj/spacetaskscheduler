...
export const useTemConstraintsListStore = defineStore("temConstraintsList", () => {
  ...
  const createTemConstraints = (tem_constraint_task1 = Number , tem_constraint_task2 = Number, tem_constraint_type = '',minimum_interval_time = '', min_timing_start_point = '', maximum_interval_time = '', max_timing_start_point = '', tem_constraint_note = '') => {
    return {
      key: nanoid(),
  ...
  const addTemConstraints = (newTemConstraints) => {
    temConstraintsList.value.push(createTemConstraints(
  ...
  const updateTemConstraints = (index, newTemConstraints) => {
    Object.assign(temConstraintsList[index], newTemConstraints);
  }

  // 删除一个 temConstraints 对象
  const removeTemConstraints = (index) => {
