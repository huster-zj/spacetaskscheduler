  ...
  mounted() {
    ...
    gantt.templates.tooltip_text = (start, end, task) => {
      ...
      return `<b>任务:</b> ${task.text}<br/>
              <b>计划时间窗口:</b> ${formatDate(new Date(task.earliestStart))} - ${formatDate(new Date(task.latestEnd))}<br/>
              <b>可行时间窗口:</b> ${formatDate(new Date(task.availableStart))} - ${formatDate(new Date(task.availableEnd))}<br/>
              <b>调度后时间窗口:</b> ${formatDate(new Date(task.startDate))} - ${formatDate(new Date(task.endDate))}<br/>`;
    };

    gantt.templates.task_text = function (start, end, task) {
      return renderLabel(task.progress1, 'nearly_done', task.left1) +
        renderLabel(task.progress2, 'in_progress', task.left2)
    };
