...
| 访问控制 | 提供前端登录门禁；默认账号为 `admin`，默认密码为 `123456` |
...
1. 启动前端和后端。
2. 打开前端地址，在登录页输入账号 `admin` 和密码 `123456`。
3. 点击页面右上角的下载图标。
4. 选择“2026 综合演示”并下载 `.sts`。
5. 点击顶栏“打开”，选择刚下载的规划包。
6. 依次查看属性、资源、任务和约束。
7. 打开“运行”，保持默认启发式算法并点击“运行”。
8. 系统完成预处理和算法计算后会自动进入调度结果页。
9. 在“主视图”查看资源/任务甘特图，在“报告”中查看并下载分析结果。
...
首次打开前端会进入登录页。当前原型使用公开的默认演示凭据：账号 `admin`，密码 `123456`。登录状态只保存在当前浏览器标签页的 `sessionStorage` 中，关闭标签页后失效。
...
- 系统当前没有后端用户账号、数据库、权限控制、云端协作或服务端项目管理。前端登录仅用于演示环境的访问门禁，默认凭据公开，不应视为生产级身份认证。
